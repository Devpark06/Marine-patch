...
export(Color) var surface_color = Color(0.67, 0.85, 0.90, 1.00)
...
export(float) var wave_speed = 5.0 #How quick they are
export(float) var wave_width = 32.0 #How wide they are
...
export(int) var point_per_distance = 20 #A point will be created every nth units along the water's surface. More points means higher fidelity
...
onready var water_point_scene := preload("res://Scenes/Water/WaterPoint.tscn") #The scene for the points along the water's surface
...
func _physics_process(delta) -> void:
	...
	if collision_polygon != null:
		...
		for i in range(points.size()):
			...
			for _j in range(wave_spread_amount):
				#Point to Left
				if i - 1 >= 0:
...
func _draw() -> void:
	...
	if collision_polygon != null:
		...
		if draw_water:
			...
			$Polygon2D.set_polygon(PoolVector2Array(polygon))
			$Line2D.points = PoolVector2Array(surface)
			$Line2D.default_color = surface_color
